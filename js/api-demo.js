...
async function pseudoFingerprint(publicKeyBytes) {
  ...
  return Array.from(mixed.slice(0, 16), (b) =>
...
const preview = (s) =>
  s.length > 160 ? `${s.slice(0, 160)}… (${s.length} chars)` : s;
...
btn.addEventListener("click", async () => {
  ...
  if (!text) {
    setResult("demo-out", "Please enter some text.");
    return;
  }
  ...
  try {
    ...
    if (res.ok && json.ok) {
      setResult(
        "demo-out",
        `Mode: ${json.mode}, Transformed text: ${json.transformed}`,
      );
